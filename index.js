...
function findElementGreaterThan(arr, num) {
  ...
  // Використовуємо метод find для пошуку першого елементу, який є більшим за задане
  //число num.
...
function findIndexByProperty(arr, name) {
  ...
  // Використовуємо метод findIndex для пошуку індексу об'єкта, властивість
  // 'name' якого співпадає з заданим іменем.
...
function getSubArrayAndConvertToString(arr, startIdx) {
  ...
  // За допомогою методу slice() отримуємо підмасив від заданого індексу до
  // кінця масиву.
...
function findLastNegativeNumberIndex(arr) {
  // Перевіряємо, чи вхідний параметр є масивом, якщо ні, повертаємо -1
  // Використовуємо метод findLastIndex() для знаходження індексу останнього
  // входження від'ємного числа
  // Повертаємо індекс останнього від'ємного числа
  if (!Array.isArray(arr)) {
    return -1;
  }
  const lastIndex = arr.findIndex((num) => num < 0);
  return lastIndex;
}
